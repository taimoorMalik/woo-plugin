...
function wq_add_cutting_list_button() {
    ...
    $intersect = array_intersect($allowed_cats, $product_cats);

    // Also check parent categories
    if (empty($intersect)) {
        foreach ($product_cats as $cat_id) {
            $ancestors = get_ancestors($cat_id, 'product_cat');
            if (array_intersect($allowed_cats, $ancestors)) {
                $intersect = true;
                break;
            }
        }
    ...
    if (!empty($intersect)) {
        $is_single = current_action() === 'woocommerce_after_add_to_cart_form';
        ...
        if ($product->is_type('variable')) {
            if ($is_single) {
                ...
                jQuery(document).ready(function($) {
                    ...
                    $(document).on('click', '.wq-cutting-list-btn', function(e) {
                        var href = $(this).attr('href');
                        if (href === '#' || href.endsWith('#')) {
                            e.preventDefault();
                            alert('Please select some product options before adding to the cutting list.');
                        }
                    });
                });
                </script>";
